...
class AwesomeCacheManagerTests: XCTestCase {
    ...
    func testSaveCache() {
        ...
        let dataFromCache1 = cacheManager.verifyForCache(withUrl: url, method: method, body: body)
        XCTAssertNil(dataFromCache1)
        
        cacheManager.saveCache(withUrl: url, method: method, body: body, data: data)
        
        let dataFromCache2 = cacheManager.verifyForCache(withUrl: url, method: method, body: body)
    ...
    func testSaveCacheWithParams() {
        ...
        let dataFromCache3 = cacheManager.verifyForCache(withUrl: url, method: method, body: nil)
        XCTAssertNil(dataFromCache3)
        
        let dataFromCache4 = cacheManager.verifyForCache(withUrl: url, method: .POST, body: body)
        XCTAssertNil(dataFromCache4)
        
        let dataFromCache5 = cacheManager.verifyForCache(withUrl: UUID().uuidString, method: method, body: body)
        ...
        cacheManager.saveCache(withUrl: url2, method: method2, body: body2, data: data2)
        
        let dataFromCache6 = cacheManager.verifyForCache(withUrl: url2, method: method2, body: body2)
    ...
    func testURLCacheKey() {
        ...
        let key = AwesomeCacheManager.buildURLCacheKey(url, method: method, bodyData: nil)
        XCTAssertEqual(key, hashKey)
        
        let body: Data? = UUID().uuidString.data(using: .utf8)
        let hashKeyWithBody = url + "?keyHash=\(String(data: body!, encoding: .utf8)! + url + method.rawValue)"
        let keyWithBody = AwesomeCacheManager.buildURLCacheKey(url, method: method, bodyData: body)
